...
- **`funk` DRUM_PATTERNS key = UK Garage** — the button now reads "UK Garage" (`aria-label`) but the internal style key is still `funk` (so old saved sequences load). `funk` also gets a heavy 0.16 swing via the per-style `SWING` map in `playDrumsAtStep` (techhouse 0.08). Don't "fix" the key name.
