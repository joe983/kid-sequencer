...
30. **A4 colouring-in worksheet print** (2026-06-07) — Print button now prints a worksheet (`#printSheet`, built once by `buildPrintWorksheet`) that mirrors the on-screen sequencer in B&W: title + one-line instruction, 5 decorative note-length circles down the left aligned to the grid's height, beat numbers (1–4) above each group of 4 columns, and a uniform 8×16 grid of rounded thick-bordered cells. Layout is a 2×2 CSS grid (`grid-template-columns: 14mm 4mm 1fr; grid-template-rows: auto auto`) so the side col sits in row 2 only (matching grid height, not body height) and the beat row sits in col 3 only (matching grid columns, not side col). `@media print { @page { size: A4 landscape; margin: 10mm 5mm } }` — horizontal margin minimised because the 2:1 grid is always width-constrained on A4 landscape; vertical margin doesn't affect cell size so kept at 10mm for printer safety. Cell size ≈ 17mm. The grid itself stays a uniform rows×cols so the camera sheet-scan crop still works — all decoration sits outside it.
...
### CSS cache busting
The `<link>` tag uses `css/styles.css?v=N`. Bump `N` on every deploy that changes styles.css (currently `?v=22`).
...
- **Print worksheet (`#printSheet`) grid must stay uniform for camera scan** — `buildPrintWorksheet()` builds a 2×2 CSS-grid layout: row 1 = beat numbers (col 3 only), row 2 = side col of 5 decorative note-length circles (col 1) + the 8×16 grid (col 3). The grid itself MUST stay a regular `repeat(16, 1fr) × repeat(8, 1fr)` of bordered cells — the camera sheet-scan crop expects a uniform rows×cols of cells. Decorative elements (title, instructions, side circles, beat numbers) live OUTSIDE the grid and are fine; do not add markers inside the grid (e.g. beat-divider lines between cells) or the scanner breaks. CSS lives under `@media print` in styles.css; `@page { size: A4 landscape; margin: 10mm 5mm }` minimises horizontal margin only (grid is always width-constrained on 2:1 A4 landscape).
