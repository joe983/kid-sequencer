...
public/
  ...
  css/styles.css      ← extracted styles (linked from index.html, currently ?v=68)
...
58. **Five-note (C–G) beginner view + view toggle — PLANNED, NOT BUILT (2026-08-02)** — the owner asked for an alternative sequencer screen for younger learners: only five notes (C–G), correspondingly bigger rows, the note-length notation icon drawn inside each placed block, and a small button below the note-length circles to switch views. **Nothing is implemented — no code was changed.** The approved, adversarially-checked implementation plan is committed at [`docs/specs/2026-08-02-five-note-view-plan.md`](docs/specs/2026-08-02-five-note-view-plan.md); **read it before touching any of this.** Headlines a future session needs: it is **NOT** a `?level=N` learning level (that machinery strips drums/save/print/tempo/key/AI/pots and forces cols:8 — wrong vehicle); it is the full app with a shorter grid. **The 5-note view is exactly the BOTTOM FIVE ROWS of the existing grid** — `freqs` is indexed top-down so C,D,E,F,G are already model indices 7,6,5,4,3, which means `rowColors[3..7]` is already the blue/green/yellow/orange/red C=red palette the lesson levels use, and `pitchFor()` needs **zero** changes because `SCALES[key]` is reversed with index 7 = root (the key picker keeps transposing correctly). So the feature is a **render/playback window over an unchanged 8×16 model**: `notesByRow`/`occ` stay 8 rows forever, no parse-time `const` becomes mutable, and the owner's "hide the top 3 rows non-destructively, restore on switch back" requirement falls out for free. Glyphs derive from `note.len` via a `NOTE_ART` map built from `toolSteps` — **never** from `note.kind` (that would drag in the `cloneState` kind-drop bug, the cloud-save/scan kind gaps, `tick`'s `kind==="eighth"` branch, and the learning-mode glyph CSS). Four measured facts the plan depends on: `--cell` goes 64→**78** (the horizontal centring solve caps at 79.375, and `fitToViewport`'s own `maxCellByWidth` is **1px optimistic** because `outerPadding` is 24 vs a real 28 and `gridExtra` is 20 vs the shell's real 32 — today the `MAXCELL=64` cap hides this); `#toolsList` has **no `align-items`**, so shrinking `.tool` to 108px makes the circles sit LEFT in the 124px column unless `align-items:center` is added; `--faderTrackH` ([index.html](public/index.html) ~7922) **hardcodes 8 rows** (`cell*8 − 322`), so it must be pinned to 190px in the 5-note view or the right column grows 112px; and the **gating check before anything else is `KidSequencer.Scan.printMarkInsets()`** — `_applyPrintScale` measures `#toolsList` in its bbox, so shrinking the circles could move the printed layout and invalidate the committed `SHEET_GEOMETRY` insets, breaking the live worksheet scanner. Owner decisions already settled: glyphs in the 5-note view only, hide-and-restore (never delete), Print hidden in the 5-note view (scanner untouched, a `kidseq:main5` sheet template is deferred), toggle below the circles, free for all tiers, default 8-row, choice remembered in `localStorage['kidseq_note_view']`.
