...
51. **Engine R32 — producer SOUND pass, techhouse (PLANNED 2026-07-15, NOT yet built)** — owner listened to the R31 producer battery and rejected it: **"they all sound the same."** Verified root cause: R31 varied per-press *decisions* but every producer shares ONE drum kit (`KITS` genre-keyed), one synth family (all Surge param-dicts), zero sampled FX (all risers/impacts synthesized the same), and the varied leads sit ≥8 dB under the kid's identical riff. **Lesson recorded: decision logs are NOT sound — verify producer variety with audio-level evidence (spectral distinctness), not `producer=` log lines.** R32 gets REAL distinct sound SOURCES into the engine per producer (the drums.pack/engine_extras.pack way): 6 producer drum kits (`KITS["techhouse:<producer>"]` + `kit_key()` helper + threading through the AUDIO drum calls in `arrange/render.py::build_song` + `kick_slot_hz`/`master()` slot cache), a new `smp_render.py` repitch one-shot sampler (real vocal chops / funk stabs / chants — owner approved), producer Surge-patch v2 + a generalized `VOICE_POST` pedalboard-chain hook (extends R31's `PAD_POST`), sampled FX one-shots (`fx_samples.py`), small per-producer mix colour, and an **automated spectral distinctness gate** (`test_producer_sound.py`) so "they all sound the same" fails CI. Sources = owner's commercial packs (AA Vengeance Essential House, [VB] Hyperfunk, musicradar carnival-rave/rave, TR-909, Madeon) under `C:/Users/Joe_C/Documents/MyMusic/Samples/` — committed into `engine/packs/producer_techhouse.pack`, LICENSES.md records owner-provided. **Full plan (8 increments R32a–h, file paths, pack schema, distinctness thresholds, 3 owner-listen checkpoints): `~/.claude/plans/ok-now-back-to-bubbly-allen.md`.** Same deploy gate (R1–R32 together). Also this session: `/doctor` trimmed CLAUDE.md ~5.7k tokens/session (items 1–40 → `CLAUDE-ARCHIVE.md`).
