...
59. **Stripe LIVE + the upgrade modal made usable on a phone (2026-08-04, MERGED to `main` → LIVE)** — **the app now takes real money.** (A) **Go-live** (`f1f11bb`, `617c93d`): live products/prices created, live Customer portal `bpc_1U0dkbFLX0F4HP6XW08I3U8f`, live webhook `we_1U0dkbFLX0F4HP6Xd8xZb8EY`, both secrets rotated in Secret Manager, functions redeployed, and `npm run check:stripe -- --probe` green. **The FULL loop is proven on live:** a real card was charged and the webhook wrote `tier:"paid"` + live `cus_`/`sub_` IDs to `users/{uid}` (09:01), two AI tracks were generated against the live engine and quota (`aiUsedThisMonth:2`), and an immediate cancellation flipped the same doc to `tier:"free"` (09:28). **Cancelling does NOT delete the user doc** — the handler does `set({tier:"free"},{merge:true})`, so `stripeCustomerId` and the AI counters survive. Note the portal is configured `subscription_cancel.mode:"at_period_end"`, so `customer.subscription.deleted` fires at period end, not on the cancel click — to test a downgrade immediately, cancel from the Stripe dashboard's subscription page with the "Immediately" option. **Pausing was NOT a substitute** for a cancel test: it emits `customer.subscription.updated`, which at the time this endpoint did not subscribe to, so the tier stayed `paid` and read as a broken webhook — now handled, see #60. `setup-stripe-products.js` gained **`--write-env`**, which rewrites the six price-ID lines of `.env.kid-sequencer` itself; the hand-copy it replaces was the one manual step in the runbook and a single transposed character there produces the exact silent failure the runbook exists to prevent (functions start, pages load, only a paying customer finds out). Its main IIFE is guarded by `require.main` so `writeEnvFile` is testable against a throwaway copy without touching Stripe. **Two stale `paid` docs** in `users/` are June TEST-mode subscriptions (IDs carry the `FXTeND9GTE` account fragment; live ones carry `FLX0F4HP6X`) — not real subscribers. (B) **Upgrade modal, three bugs reported from a real iPhone** (`507d072`): **the Subscribe button was unreachable on a phone** — the card already had `overflow-y:auto` and genuinely overflowed (263px at 390×500), but the document-level `touchmove` handler cancelled the gesture (see the new gotcha below); **"Current plan" sat on the Guest card whenever the user was logged IN**, i.e. exactly when Guest is not their plan, while the Member card hid its CTA entirely — `_markCurrentTierCard()` had the ternary inverted; and the marketing view's "Already have an account? Log in" footer (now `#upgradeMarketingFooter`) showed to logged-in users. Guest's label now hides when logged in, Member reads "Current plan" (disabled, ghost-styled), and the CTAs are selected as `#tierCardGuest .tierCta` / `#tierCardMember .tierCta` rather than by `.tierCta-ghost`, because Member now borrows that class. Verified in-browser across guest → member → pro → member → guest with no sticky state. No CSS change, so no `?v` bump. The touch fix was exercised with synthetic events and a forced `stageScaled` class (**a desktop pane never sets that class itself**) and then **confirmed on the owner's real iPhone the same day** — the modal scrolls and Subscribe is reachable.

60. **A subscription that stops paying now loses Pro (2026-08-04, deployed)** — closes the gap left by #59. The webhook only listened for `customer.subscription.deleted`, but a subscription can stop paying without ever being deleted: Stripe's retry schedule leaves a failed renewal in `past_due` for **weeks** before it becomes `unpaid`/`canceled`, and a paused subscription stops collecting indefinitely while its status stays `active`. Access was kept through all of it. Now `customer.subscription.updated` is handled, and the entitlement rule lives in its own firebase-admin-free module **`functions/subscription-state.js::isPayingSubscription(sub)`** — only `active`/`trialing` and never while `pause_collection` is set. Two things deliberately do NOT revoke: `cancel_at_period_end` (status stays `active` to the end of the paid period — they bought the month) and a successful retry (it fires the same event back to `active`, so access restores itself with no special-casing). Both subscription handlers now go through `setTierByCustomer(customerId, tier, reason)`, which looks the user up by the `stripeCustomerId` the first webhook stored (the uid only exists on the original checkout session) and **writes only on an actual change** — Stripe emits `updated` for card edits, metadata and `cancel_at_period_end` toggles, and re-writing `paid` over `paid` each time is pure churn. Gated by `npm test` in `functions/` (18 checks: every Stripe status, paused-vs-null `pause_collection`, `cancel_at_period_end`, and malformed input, which must never grant access). **The endpoint subscribes to all three events as of 2026-08-04** — `we_1U0dkbFLX0F4HP6Xd8xZb8EY` now lists `checkout.session.completed`, `customer.subscription.deleted` and `customer.subscription.updated`. Stripe only delivers what an endpoint lists, so the handler was dead code until this was added. ⚠️ `enabled_events` is a **full replacement** on update — always read the current list and extend it; sending a bare array silently drops the two events fulfilment depends on. ⚠️ `npm run check:stripe` does NOT verify the third event; it only checks the original two, so a future endpoint rebuild can drop it without failing the pre-flight. **Still unexercised in production:** no real subscription has yet gone `past_due` or paused, so this path has passed its unit tests but has never run on a live event.

61. **An AI track you never received is no longer charged for (2026-08-04, deployed)** — reported from real use: a generation errored, the owner restarted it, no track was produced and a credit was gone. The refund path existed (`generateAiTrack` consumed a credit up front and refunded it in a `catch`) but it can only run **if the process survives to run the catch** — and the ways this function dies are precisely the ways it doesn't: the 540 s `timeoutSeconds` cap, an OOM at 512 MiB while holding the MP3 buffer, or the instance being torn down mid-render. A refund is the wrong mechanism for a failure mode that stops the code executing. **The credit is now committed only after the track is in Storage** (`aiUsedThisMonth` / `aiTopupBalance` written in a transaction immediately before the function returns the URL); the entry transaction only *checks* the balance. Nothing to refund means no crash path can charge. If that final charge write itself throws, it is caught and logged and the track is still returned — failing a request whose work is already delivered is worse than one free track (grep `charge failed, track delivered free`). **Failures being free needs its own brake**, or a broken account can retry a two-minute Modal render forever, so the second half is **`functions/ai-attempts.js`** (firebase-admin-free + unit-tested, same shape as `subscription-state.js`): (1) **one generation in flight per user** — `aiInFlightAt` claimed at entry, released at the end, which both rejects the reload-and-retry that triggered this report and closes the check-at-start/charge-at-end window that would otherwise let concurrent calls all pass one balance check; (2) **`MAX_CONSECUTIVE_FAILURES` 5 → `FAILURE_COOLDOWN_MS` 15 min**, streak reset by any success; (3) **`MAX_FAILURES_PER_MONTH` 40** as the backstop for failures spaced far enough apart to keep clearing the cooldown (successes need no ceiling — credits already bound them). ⚠️ **The in-flight marker MUST expire and does — `IN_FLIGHT_TTL_MS` 600 s, deliberately longer than the 540 s function timeout.** A killed instance never reaches the release, so without expiry the first crash would lock a user out of the feature permanently, which is worse than the bug being fixed; a test pins `IN_FLIGHT_TTL_MS > 540 s`. Client: `functions/failed-precondition` → "still being made", `functions/unavailable` → cooldown, and **every failure message now says "You haven't been charged"**, because being charged for nothing is the actual complaint. Gated by `npm test` in `functions/` (now 18 + 40 checks; `test-ai-attempts.js` covers stale-marker recovery, streak/cooldown/ceiling boundaries, month rollover, and garbage state in both directions). **Unexercised in production:** no real run has yet hit the cooldown or the ceiling.
...
- **Locked tap → full tier modal is the INTENDED design — do not re-raise it as a problem.** The owner confirmed this explicitly on 2026-08-04 ("This is how I wanted it") after rolling back a redesign that changed it. Locked controls ARE the discovery surface, and the route is complete: Print/Save/Load open the modal via `bindLockedNudge`, and the AI button calls `openUpgradeModal('subscribe')` directly when `!isPaid` ([index.html](public/index.html):1303) — so a logged-in free Member reaches the tier comparison from any Pro feature they actually reach for, at the moment they want it, rather than via a standing upsell. An earlier note here claimed Members "have no path to discover Pro"; that was **wrong**, and it survived in this file long enough to be repeated back to the owner as a priority. The alternative redesign (account button → tier comparison; focused per-feature prompts w/ "See all plans"; persistent "Go Pro" pill; compacted cards) was built, verified and deployed to a preview channel, then **rolled back** — the plan is parked at `~/.claude/plans/this-feels-disjointed-and-joyful-wadler.md` and should only be revisited if the owner raises it. The separate *correctness* bugs in this modal were fixed 2026-08-04 (see #59).
...
- **⚠️ A document-level `touchmove` handler cancels gestures in `stageScaled` mode — any NEW scrollable overlay must be checked on a real phone.** `index.html` (~6714, in the layout IIFE) calls `e.preventDefault()` on every touchmove while `html.stageScaled` is set, to kill iOS rubber-band wiggle on the fixed stage. **`overflow-y:auto` alone therefore does NOT make a panel scrollable on a landscape phone** — this is what made the upgrade modal's Subscribe button unreachable, and the original exemption list (`input, textarea, select, option, button`) meant the only touchable escape was the very button below the fold. A `touchstart` listener now records whether the gesture began inside an ancestor that is genuinely scrollable (`scrollHeight > clientHeight` **and** computed `overflow-y` auto/scroll) and the touchmove bails on that, so overlays scroll while the stage still can't wiggle. Decided once per gesture, not per move — don't "simplify" it back into the touchmove handler, that's `getComputedStyle` ancestor-walking at 60Hz. **`stageScaled` is only applied on real touch devices, so the Browser pane and desktop Chrome never reproduce this** — force the class on to test, and treat a real-device check as the actual verification.
...
- **AI credits are charged on DELIVERY, never on entry — don't "simplify" it back to consume-then-refund** — `generateAiTrack` checks the balance at the start and writes the charge only after the MP3 is in Storage. The refund-in-`catch` it replaced silently charged for nothing on every failure that killed the process before the catch could run (540 s timeout, OOM, instance torn down). Because failures are free, three limits in `functions/ai-attempts.js` stop retry spam — one in-flight run per user, a 5-failure/15-min cooldown, and a 40-failure monthly ceiling. Any change to the in-flight marker must keep `IN_FLIGHT_TTL_MS` above the function's `timeoutSeconds`, or a crashed run locks the user out of the feature for good.
